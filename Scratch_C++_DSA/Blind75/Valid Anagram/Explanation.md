...
The T.C of this approach will be -> O(n) => O(n) for the iteration of the strs. 
The S.C of this approach will be -> O(n) + O(n) = O(n) => we are using extra space here for storing each char and its freq (map).
